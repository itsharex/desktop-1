...
const ResolveContainer = (props: ResolveContainerProps) => {
    ...
    const createContainer = async (port: number) => {
        ...
        if (props.devType == "vscode") {
            ...
            console.log("devc", "container", "create", "--localPath", pathInDocker, "--image", dockerImage, "--localPort", `${portInDocker}`, props.repoId, props.repoPath, `${port}`);
            const cmd = Command.sidecar("bin/devc", ["container", "create", "--localPath", pathInDocker, "--image", dockerImage, "--localPort", `${portInDocker}`, props.repoId, props.repoPath, `${port}`]);
            const output = await cmd.execute();
            const obj = JSON.parse(output.stdout) as CommandResult;
            if (obj.success == true) {
                return obj.data as string;
