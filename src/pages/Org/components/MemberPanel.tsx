...
const MemberPanel = (props: MemberPanelProps) => {
    ...
    const loadOkrList = async () => {
        const res = await request(list_okr({
            session_id: userStore.sessionId,
            org_id: orgStore.curOrgId,
            member_user_id: props.curMember.member_user_id,
    ...
    }, [props.curMember.member_user_id]);
    ...
    return (
        <Card title={props.curMember.display_name}
            ...
            extra={
                <>
                    {userStore.userInfo.userId == props.curMember.member_user_id && (
                        <Button type="primary" onClick={e => {
                            e.stopPropagation();
                            e.preventDefault();
        ...
        >
            ...
            {showCreateModal == true && (
                <EditModal onCancel={() => setShowCreateModal(false)} onOk={() => {
                    if (curPage != 0) {
                        setCurPage(0);
                    } else {
                        loadOkrList();
